...
class FilterModel(torch.nn.Module):
    ...
    algorithm.
    """
    def __init__(self, dimensionIn=1, dimensionHidden=64, dimensionOut=1, numLayers=1, batchSize=1):
        super().__init__()
        ...
        self._lstm = torch.nn.LSTM(dimensionIn, dimensionHidden, batch_first=True, num_layers=numLayers)
        self._hiddenToPrediction1 = torch.nn.Linear(dimensionHidden // 1, dimensionHidden // 2)
        self._hiddenToPrediction2 = torch.nn.Linear(dimensionHidden // 2, dimensionHidden // 4)
        self._hiddenToPrediction3 = torch.nn.Linear(dimensionHidden // 4, dimensionOut)

    def resetSequence(self, batchSize=None):
        if batchSize:
            self._batchSize = batchSize
        self._hidden = (torch.zeros(self._numLayers, self._batchSize, self._dimensionHidden),
                        torch.zeros(self._numLayers, self._batchSize, self._dimensionHidden))
